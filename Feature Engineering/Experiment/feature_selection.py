...
def _family_of(feat: str) -> str:
    """Return the configured feature family for one feature name."""
...
def _feat_type(s: pd.Series) -> str:
    """Classify a Series as categorical, binary, or numeric."""
...
def _sub_idx(n: int, seed: int) -> np.ndarray | None:
    """Return a random index subset when MI should run on a sample only."""
    if n <= MI_SUBSAMPLE:
        return None
...
def _safe_spearman(x: pd.Series, y: pd.Series) -> float:
    """Compute Spearman correlation with a minimum valid-row guard."""
    m = x.notna() & y.notna()
    if m.sum() < 20:
        return np.nan
...
def _safe_chi2(x: pd.Series, y: pd.Series, n_bins: int = 10) -> float:
    """Compute a chi-square score after binning numeric inputs when needed."""
    m = x.notna() & y.notna()
    if m.sum() < 20:
        return np.nan
    xv, yv = x[m], y[m]
...
def compute_filter_report(
    ...
    """Score each feature with missingness, variance flags, and signal metrics."""
    n_rows = len(X)
...
def find_redundant_pairs(X: pd.DataFrame) -> pd.DataFrame:
    """Find highly correlated numeric feature pairs above the threshold."""
...
def build_family_report(
    ...
    """Aggregate filter findings from feature level to family level."""
...
def _build_safe_vs_cond(cls_all: pd.DataFrame, reg_all: pd.DataFrame) -> str:
    """Summarize how safe and conditional feature families compare by stage."""
    lines: list[str] = [
        "",
        "=" * 72,
...
def _build_summary(
    ...
    """Assemble the human-readable summary text for all filter outputs."""
    L: list[str] = [
        "=" * 72,
...
def _load(name: str) -> pd.DataFrame:
    """Load one saved parquet matrix from the feature-selection input folder."""
...
def run_feature_selection() -> None:
    """Run the complete Round-3 filter analysis and export all reports."""
    t0 = time.time()
...
def main() -> None:
    """Parse CLI arguments and start the filter-based feature selection."""
